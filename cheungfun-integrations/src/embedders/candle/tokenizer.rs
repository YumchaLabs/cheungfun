...
impl EmbeddingTokenizer {
    ...
    pub fn from_tokenizer(
        ...
        let pad_token_id = vocab.get("[PAD]")
            .or_else(|| vocab.get("<pad>"))
            .or_else(|| vocab.get("</s>"))
            .copied()
            .unwrap_or(0);
        
        let cls_token_id = vocab.get("[CLS]")
            .or_else(|| vocab.get("<s>"))
            .copied();
        
        let sep_token_id = vocab.get("[SEP]")
            .or_else(|| vocab.get("</s>"))
            .copied();
    ...
    pub fn to_tensors(
        ...
        // Convert to tensors
        let input_ids = Tensor::from_vec(all_input_ids, (batch_size, seq_len), device)

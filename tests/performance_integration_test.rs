...
async fn test_simd_performance() {
    ...
    println!("📊 Basic scalar: {:?} ({:.2} ops/sec)", 
             basic_time, num_pairs as f64 / basic_time.as_secs_f64());
    ...
    #[cfg(feature = "simd")]
    {
        ...
        if simd_ops.is_simd_available() {
            ...
            println!("⚡ SIMD operations: {:?} ({:.2} ops/sec)", 
                     simd_time, num_pairs as f64 / simd_time.as_secs_f64());
...
async fn test_vector_store_performance() {
    ...
    println!("  ⏱️  Time: {:?} ({:.2} queries/sec)", 
             basic_time, num_queries as f64 / basic_time.as_secs_f64());
    ...
    println!("  ⏱️  Time: {:?} ({:.2} queries/sec)", 
             optimized_time, num_queries as f64 / optimized_time.as_secs_f64());
    println!("  📈 Speedup: {:.2}x", 
    ...
    #[cfg(feature = "hnsw")]
    {
        ...
        println!("  ⏱️  Time: {:?} ({:.2} queries/sec)", 
                 hnsw_time, num_queries as f64 / hnsw_time.as_secs_f64());
        println!("  📈 HNSW speedup vs basic: {:.2}x", 
                 basic_time.as_nanos() as f64 / hnsw_time.as_nanos() as f64);
        println!("  📈 HNSW speedup vs optimized: {:.2}x", 
                 optimized_time.as_nanos() as f64 / hnsw_time.as_nanos() as f64);

        let stats = hnsw_store.get_stats();
        println!("  📊 HNSW stats: {} vectors, {:.2}μs avg search", 
...
async fn test_parallel_processing() {
    ...
    #[cfg(feature = "simd")]
    {
        ...
        for vector in &vectors[1..] {
            sequential_results.push(simd_ops.cosine_similarity_f32(query_vector, vector).unwrap());
        }
        ...
        let start = Instant::now();
        let parallel_results = simd_ops.one_to_many_cosine_similarity_f32(query_vector, &vector_refs).unwrap();
        let parallel_time = start.elapsed();
        
        println!("📊 Operations: {}", vectors.len() - 1);
        println!("⏱️  Sequential: {:?} ({:.2} ops/sec)", 
                 sequential_time, (vectors.len() - 1) as f64 / sequential_time.as_secs_f64());
        println!("⚡ One-to-many: {:?} ({:.2} ops/sec)", 
                 parallel_time, (vectors.len() - 1) as f64 / parallel_time.as_secs_f64());
        println!("📈 Speedup: {:.2}x", 

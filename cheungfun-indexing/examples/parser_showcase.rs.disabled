...
use cheungfun_indexing::node_parser::{
    ...
    text::{
        CodeSplitter, MarkdownNodeParser, SentenceSplitter, SentenceWindowNodeParser,
        TokenTextSplitter,
    },
    NodeParser,
};
...
"#;
    ...
    for (i, node) in sentence_nodes.iter().take(2).enumerate() {
        info!(
            "节点 {}: {}",
            i + 1,
            node.content.trim().chars().take(50).collect::<String>() + "..."
        );
    }
    ...
    let markdown_nodes = <MarkdownNodeParser as NodeParser>::parse_nodes(
        &markdown_parser,
        &[Document::new(markdown_content)],
        false,
    )
    .await?;
    let duration = start.elapsed();
    ...
    let window_nodes = <SentenceWindowNodeParser as NodeParser>::parse_nodes(
        &window_parser,
        &[Document::new(text_content)],
        false,
    )
    .await?;
    let duration = start.elapsed();
    ...
    let code_nodes = <CodeSplitter as NodeParser>::parse_nodes(
        &code_splitter,
        &[Document::new(code_content)],
        false,
    )
    .await?;
    let duration = start.elapsed();
    ...
    let md_nodes =
        <MarkdownNodeParser as NodeParser>::parse_nodes(&md_parser, &[test_doc], false).await?;
    let md_duration = start.elapsed();
    info!(
        "MarkdownNodeParser: {} 节点, {:?}",
        md_nodes.len(),
        md_duration
    );

...
use cheungfun_query::utils::{
    query_utils, response_utils, QueryCache, QueryOptimizer, QueryOptimizerConfig,
    ResponsePostProcessor, ResponsePostProcessorConfig, CitationFormat,
};
...
fn test_query_builder_pattern() {
    ...
    assert_eq!(query.filters.get("category"), Some(&serde_json::Value::String("science".to_string())));
}

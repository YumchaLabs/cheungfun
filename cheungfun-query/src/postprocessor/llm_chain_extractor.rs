...
use cheungfun_core::{types::ScoredNode, Result};
...
impl LLMChainExtractor {
    ...
    async fn extract_single_node(&self, node: &ScoredNode, query: &str) -> Result<String> {
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                cheungfun_core::CheungfunError::external(anyhow::anyhow!(
                    "LLM extraction failed: {}",
                    e
                ))
            })?;

        let extracted_content = response
            .content_text()
            .unwrap_or_default()
            .trim()
            .to_string();
    ...
    async fn extract_batch_nodes(&self, nodes: &[ScoredNode], query: &str) -> Result<Vec<String>> {
        ...
        debug!(
            "Extracting content from {} nodes in batch with LLM",
            nodes.len()
        );
        ...
        let batch_response = response.content_text().unwrap_or_default().trim();
...
impl LLMChainExtractorBuilder {
    ...
    pub fn build(self) -> Result<LLMChainExtractor> {
        let llm_client = self.llm_client.ok_or_else(|| {
            cheungfun_core::CheungfunError::configuration("LLM client is required".to_string())
        })?;

...
#[cfg(feature = "hnsw")]
use cheungfun_integrations::vector_stores::hnsw::{HnswConfig, HnswVectorStore};
...
async fn test_simd_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(feature = "simd")]
    {
        ...
        let query_vector: Vec<f32> = (0..dimension).map(|i| (i as f32 * 0.001).cos()).collect();
        ...
        println!(
            "   Throughput: {:.2} similarities/ms",
            num_vectors as f64 / simd_duration.as_millis() as f64
        );
        println!(
            "   Results sample: {:.6}, {:.6}, {:.6}",
...
async fn test_hnsw_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(feature = "hnsw")]
    {
        ...
        println!(
            "   Avg per query: {:?}",
            search_duration / num_queries as u32
        );
        println!(
            "   Queries/sec: {:.2}",
            num_queries as f64 / search_duration.as_secs_f64()
        );
...
async fn test_memory_optimization() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(feature = "optimized-memory")]
    {
        ...
        // Test search performance
        let query_embedding: Vec<f32> = (0..dimension).map(|i| (i as f32 * 0.001).cos()).collect();
...
async fn test_combined_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   Search time ({} queries): {:?}",
        num_queries, baseline_search_duration
    );
    println!(
        "   Avg per query: {:?}",
        baseline_search_duration / num_queries as u32
    );
    ...
    #[cfg(all(feature = "hnsw", feature = "optimized-memory"))]
    {
        ...
        println!(
            "   Search time ({} queries): {:?}",
            num_queries, optimized_search_duration
        );
        println!(
            "   Avg per query: {:?}",
            optimized_search_duration / num_queries as u32
        );

        // Calculate improvements
        let add_speedup =
            baseline_add_duration.as_secs_f64() / optimized_add_duration.as_secs_f64();
        let search_speedup =
            baseline_search_duration.as_secs_f64() / optimized_search_duration.as_secs_f64();

        println!("📈 Performance Improvements:");
        println!("   Add speedup: {:.2}x", add_speedup);
        println!("   Search speedup: {:.2}x", search_speedup);
        println!(
            "   Overall efficiency gain: {:.1}%",

...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("📏 文件大小: {} 字符, {} 行", content.len(), content.lines().count());
    println!();
...
async fn analyze_ast_structure(content: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, class) in analysis.classes.iter().enumerate() {
        println!("  {}. {} (行 {}-{})", i + 1, class.name, class.start_line, class.end_line);
    ...
    for (i, func) in analysis.functions.iter().take(10).enumerate() {
        println!("  {}. {} (行 {}-{})", i + 1, func.name, func.start_line, func.end_line);
...
async fn test_chunking_strategies(content: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, strategy) in strategies {
        ...
        println!("   参数: {} 行/块, {} 行重叠, {} 字符上限", chunk_lines, overlap, max_chars);
        ...
        if name == "🚀 最优策略" {
            println!("\n📋 最优策略详细分块内容:");
            println!("================================");
            for (i, chunk) in ast_chunks.iter().enumerate().take(10) { // 限制最多显示10个块
                ...
                for (j, line) in lines.iter().take(3).enumerate() {
                    println!("   {:3}: {}", j + 1, line.chars().take(80).collect::<String>());
                }
...
fn analyze_chunk_quality(chunks: &[String], strategy_name: &str) {
    ...
    let avg_chars = if chunks.is_empty() { 0 } else { total_chars / chunks.len() };
    ...
    // 分析代码结构完整性
    let complete_functions = chunks.iter().filter(|chunk| {
    ...
    let has_class_definition = chunks.iter().filter(|chunk| {
        chunk.contains("class ") || chunk.contains("public class")
    }).count();
    
    println!("     {} 质量分析:", strategy_name);
    println!("       - 平均字符数: {}", avg_chars);
    println!("       - 平均行数: {:.1}", avg_lines);
    println!("       - 完整函数块: {}/{}", complete_functions, chunks.len());
...
async fn compare_chunking_quality(content: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 使用当前配置
    let config = CodeSplitterConfig::new(
    ...
    for (i, chunk) in chunks.iter().take(5).enumerate() {
        ...
        let has_function = chunk.contains("public ") || chunk.contains("private ") || chunk.contains("void ");
        let has_comments = chunk.contains("//") || chunk.contains("/*");
        let open_braces = chunk.matches('{').count();
        let close_braces = chunk.matches('}').count();
        let brace_balanced = open_braces == close_braces;
        
        println!("结构: 类={}, 函数={}, 注释={}, 括号平衡={}", 

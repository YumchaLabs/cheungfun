...
async fn test_parallel_vector_operations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(feature = "optimized-memory")]
    {
        ...
        println!("✅ Parallel indexing completed in {:?}", parallel_add_duration);
        ...
        println!("✅ Parallel search completed in {:?}", parallel_search_duration);
        ...
        println!("   Avg per query: {:?}", batch_duration / queries.len() as u32);
    }
    
    #[cfg(not(feature = "optimized-memory"))]
    {
        println!("⚠️  Parallel vector operation tests skipped");
        println!("   💡 Enable with: cargo run --features \"performance\" --bin simple_parallel_test");
    }
...
async fn test_performance_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    
    {
        ...
        println!("   Sequential: {:?} ({:.2} ops/sec)", 
                sequential_duration, 
                iterations as f64 / sequential_duration.as_secs_f64());
        println!("   Parallel:   {:?} ({:.2} ops/sec)", 
                parallel_duration, 
        ...
        println!("   Batch parallel: {:?} ({} batches of {})", 

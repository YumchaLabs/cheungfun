...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  Individual processing: {:?} ({:.2} texts/sec)",
        individual_time, individual_rate
    );
    println!(
        "  Batch processing:      {:?} ({:.2} texts/sec)",
        batch_time, batch_rate
    );
    println!(
        "  Speedup:               {:.2}x",
    ...
    println!(
        "✅ Processed {} embeddings in {:?}",
        large_embeddings.len(),
        large_time
    );

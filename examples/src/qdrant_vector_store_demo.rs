...
use cheungfun_integrations::{QdrantConfig, QdrantVectorStore};
...
async fn main() -> Result<()> {
    ...
    info!("✅ Successfully added {} nodes in {:?}", added_ids.len(), add_duration);
    ...
    if let Some(Some(mut node)) = retrieved_nodes.into_iter().next() {
        ...
        node.metadata.insert("updated".to_string(), serde_json::Value::Bool(true));
    ...
    let advanced_results = store.search_with_filter(
        vec![0.2; 384],
        5,
    ...
    info!("✅ Advanced search found {} results", advanced_results.len());

    // Step 11: Collection information
    info!("\n📋 Step 11: Getting detailed collection information");
    
    let collection_info = store.collection_info().await?;
    let collection_stats = store.collection_stats().await?;

    info!("✅ Collection details:");
    info!("   Points count: {:?}", collection_stats.get("points_count"));
    info!("   Segments count: {:?}", collection_stats.get("segments_count"));
    ...
    let batch_ids = store.batch_upsert(batch_nodes, 5).await?;
    ...
    info!("   Total insert operations: {}", final_stats.insert_operations);
    info!("   Total search operations: {}", final_stats.search_operations);
    info!("   Total update operations: {}", final_stats.update_operations);
    ...
    info!("💡 The collection '{}' remains in Qdrant for further exploration", 
...
fn create_sample_nodes() -> Vec<Node> {
    let documents = vec![
        ("Machine learning is a subset of artificial intelligence that focuses on algorithms.", "technology"),
        ("Rust is a systems programming language focused on safety and performance.", "programming"),
        ("Vector databases are specialized for storing and querying high-dimensional vectors.", "database"),
        ("Natural language processing enables computers to understand human language.", "ai"),
        ("Qdrant is a vector similarity search engine written in Rust.", "database"),
    ];
    ...
    documents
        ...
        .map(|(i, (content, category))| {
            ...
            node.metadata.insert("category".to_string(), serde_json::Value::String(category.to_string()));
            node.metadata.insert("index".to_string(), serde_json::Value::Number(i.into()));
...
fn create_batch_nodes(count: usize) -> Vec<Node> {
    (0..count)
        .map(|i| {
            ...
            let mut node = Node::new(content, source_doc_id, chunk_info);
            node.embedding = Some(embedding);
            node.metadata.insert("batch".to_string(), serde_json::Value::Bool(true));
            node.metadata.insert("batch_index".to_string(), serde_json::Value::Number(i.into()));

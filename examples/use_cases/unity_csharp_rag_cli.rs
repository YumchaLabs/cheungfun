...
//! 专为大型 Unity 项目设计，支持企业级分块策略和智能代码分析。
//!
//! ## 使用方法
//!
//! ```bash
...
impl Default for UnityCSharpRagConfig {
    fn default() -> Self {
        Self {
            ...
            embedding_provider: "fastembed".to_string(), // 默认使用 FastEmbed
...
impl UnityCSharpRagSystem {
    ...
    pub async fn build_code_index(&mut self) -> Result<()> {
        ...
        let unity_filter = FilterConfig::new()
            .with_respect_gitignore(true) // 遵循 gitignore 规则
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--embedding-provider" | "-e" => {
                if i + 1 < args.len() {
                    cli_args.embedding_provider = Some(args[i + 1].clone());
                    i += 1;
                }
            }
            "--embedding-model" | "-em" => {
                if i + 1 < args.len() {
                    cli_args.embedding_model = Some(args[i + 1].clone());
                    i += 1;
                }
            }
            "--top-k" | "-k" => {
...
fn show_help() {
    ...
    println!("  -em, --embedding-model  Embedding 模型名称");
    ...
    println!();
    println!("环境变量:");
    println!("  OPENAI_API_KEY      OpenAI API 密钥 (用于 LLM 和 OpenAI embedding)");
    println!("  GEMINI_API_KEY      Gemini API 密钥 (用于 Gemini embedding)");
    println!();
    println!("示例:");
    ...
    println!();
    println!("  # 使用 Gemini embedding");
    println!("  export GEMINI_API_KEY=\"your-key\"");
    println!("  unity_csharp_rag /path/to/unity/project --embedding-provider gemini --embedding-model gemini-embedding-001");
    println!();
    println!("  # 使用 OpenAI embedding");
    println!("  export OPENAI_API_KEY=\"your-key\"");
    println!("  unity_csharp_rag /path/to/unity/project --embedding-provider openai --embedding-model text-embedding-3-small");
}
...
async fn create_sample_unity_project(project_path: &PathBuf) -> Result<()> {
    ...
    // 创建示例 PlayerController.cs
    let player_controller_content = r#"using UnityEngine;

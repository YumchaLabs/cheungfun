...
async fn demo_pipeline_with_memory_cache() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let documents = create_sample_documents();
    let cache = Arc::new(MemoryCache::new()) as Arc<dyn PipelineCache<Error = cheungfun_core::CheungfunError>>;
    ...
    info!("Processing {} documents with memory cache...", documents.len());
    ...
    info!("📊 Memory cache stats: hit rate = {:.1}%, total entries = {}", 
          stats.hit_rate(), stats.total_entries);
...
async fn demo_pipeline_with_file_cache() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let cache = Arc::new(FileCache::new(cache_dir).await?) as Arc<dyn PipelineCache<Error = cheungfun_core::CheungfunError>>;
    ...
    info!("Processing {} documents with file cache...", documents.len());
    ...
    info!("🏥 File cache health: {:?}, hit rate = {:.1}%", 
...
async fn demo_cache_performance_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let scenarios = vec![
        ("No Cache", None),
        ("Memory Cache", Some(Arc::new(MemoryCache::new()) as Arc<dyn PipelineCache<Error = cheungfun_core::CheungfunError>>)),
        ("File Cache", Some(Arc::new(FileCache::new("./comparison_cache").await?) as Arc<dyn PipelineCache<Error = cheungfun_core::CheungfunError>>)),
    ];
...
fn create_sample_documents() -> Vec<Document> {
    vec![
        Document::new("Machine learning is a subset of artificial intelligence that focuses on algorithms."),
        Document::new("Deep learning uses neural networks with multiple layers to process data."),
        Document::new("Natural language processing helps computers understand and generate human language."),
        Document::new("Computer vision enables machines to interpret and analyze visual information."),

...
use cheungfun_indexing::node_parser::{text::MarkdownNodeParser, config::MarkdownConfig, NodeParser, TextSplitter};
...
async fn main() -> CoreResult<()> {
    ...
    let parser_documentation = MarkdownNodeParser::from_config(
        MarkdownConfig::for_documentation()
    )?;

    let parser_blog = MarkdownNodeParser::from_config(
...
"#;
    ...
    let nodes_default = <MarkdownNodeParser as NodeParser>::parse_nodes(&parser_default, &[document.clone()], false).await?;
    ...
    for (i, node) in nodes_default.iter().take(5).enumerate() {
        let header = node.metadata.get("header")
            ...
            .unwrap_or("(no header)");
        let level = node.metadata.get("header_level")
        ...
        info!("  Preview: {}...", 
              node.content.lines().next().unwrap_or("").chars().take(60).collect::<String>());
        info!("");
    }
    ...
    let nodes_shallow = <MarkdownNodeParser as NodeParser>::parse_nodes(&parser_shallow, &[document.clone()], false).await?;
    ...
    let nodes_custom = <MarkdownNodeParser as NodeParser>::parse_nodes(&parser_custom_separator, &[document.clone()], false).await?;
    ...
    let nodes_doc = <MarkdownNodeParser as NodeParser>::parse_nodes(&parser_documentation, &[document.clone()], false).await?;
    info!("Documentation config: {} nodes (max_depth=4, min_length=50)", nodes_doc.len());
    
    // Blog config  
    let nodes_blog = <MarkdownNodeParser as NodeParser>::parse_nodes(&parser_blog, &[document.clone()], false).await?;
    info!("Blog config: {} nodes (max_depth=3, separator=' > ')", nodes_blog.len());
    ...
    for (i, section) in sections.iter().take(3).enumerate() {
        let preview = section.lines().next().unwrap_or("").chars().take(50).collect::<String>();
    ...
    let _ = <MarkdownNodeParser as NodeParser>::parse_nodes(&parser_default, &[document.clone()], false).await?;
    ...
    info!("Markdown parser is {:.2}x {} than sentence parser", 
          ...
          if markdown_time > sentence_time { "slower" } else { "faster" });
    ...
    let empty_nodes = <MarkdownNodeParser as NodeParser>::parse_nodes(&parser_default, &[empty_doc], false).await?;
    ...
    let no_headers_nodes = <MarkdownNodeParser as NodeParser>::parse_nodes(&parser_default, &[no_headers_doc], false).await?;
    info!("No headers markdown: {} nodes", no_headers_nodes.len());

    // Only headers
    let headers_only_doc = Document::new("# Header 1\n## Header 2\n### Header 3");
    let headers_only_nodes = <MarkdownNodeParser as NodeParser>::parse_nodes(&parser_default, &[headers_only_doc], false).await?;
    info!("Headers only markdown: {} nodes", headers_only_nodes.len());

    // Deep nesting
    let deep_nesting = "# L1\n## L2\n### L3\n#### L4\n##### L5\n###### L6\nContent at level 6";
    let deep_doc = Document::new(deep_nesting);
    let deep_nodes = <MarkdownNodeParser as NodeParser>::parse_nodes(&parser_default, &[deep_doc], false).await?;

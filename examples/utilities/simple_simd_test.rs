...
fn test_vector_operations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(feature = "simd")]
    {
        ...
        assert!(
            (cosine_identical - 1.0).abs() < 0.001,
            "Identical vectors should have similarity ~1.0"
        );
        assert!(
            cosine_sim > 0.0 && cosine_sim < 1.0,
            "Different vectors should have similarity between 0 and 1"
        );
        assert!(
            (dot_product - 70.0).abs() < 0.001,
...
fn test_performance_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(feature = "simd")]
    {
        ...
        println!(
            "📊 Performance Results ({} iterations, {} dimensions):",
            iterations, size
        );
        println!(
            "   SIMD:   {:?} ({:.2} ops/sec)",
        ...
        println!(
            "   Scalar: {:?} ({:.2} ops/sec)",
            scalar_duration,
            iterations as f64 / scalar_duration.as_secs_f64()
        );
        ...
        let pairs: Vec<(&[f32], &[f32])> = (0..100)
            .map(|_| (vec1.as_slice(), vec2.as_slice()))
            .collect();

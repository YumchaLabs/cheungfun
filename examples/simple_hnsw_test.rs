...
async fn test_performance_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(feature = "hnsw")]
    {
        ...
        println!(
            "   Queries/sec: {:.2}",
            num_queries as f64 / hnsw_duration.as_secs_f64()
        );
        ...
        println!(
            "   Avg search time: {:.2}μs",
            final_stats.avg_search_time_us
        );
        println!(
            "   Memory usage: {:.2} MB",
        ...
        println!(
            "   Estimated linear search time: {:?}",
            theoretical_linear_time
        );
        println!("   Estimated speedup: {:.2}x", speedup);
        println!(
            "   Search efficiency: {:.2}%",
            final_stats.search_efficiency * 100.0
        );
    }

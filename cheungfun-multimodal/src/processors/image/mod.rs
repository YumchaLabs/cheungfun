...
impl ImageProcessor {
            ...
            if image.width() > max_w || image.height() > max_h {
                return Err(crate::error::MultimodalError::validation(
                    format!("Image dimensions {}x{} exceed maximum {}x{}", 
                           image.width(), image.height(), max_w, max_h)
                ));
            }
        }
    ...
    async fn convert_image_format(
        ...
        let format = match target_format {
            ...
            _ => return Err(crate::error::MultimodalError::unsupported_format(
    ...
    fn extract_image_features(&self, image: &DynamicImage) -> HashMap<String, serde_json::Value> {
        ...
        features.insert("aspect_ratio".to_string(), 
        ...
        features.insert("channel_count".to_string(), color_type.channel_count().into());
        ...
        if !pixels.is_empty() {
            ...
            if let Some(((r, g, b), _)) = color_counts.iter().max_by_key(|(_, count)| *count) {
                features.insert("dominant_color".to_string(), 
...
impl MediaProcessor for ImageProcessor {
    ...
    async fn process(&self, content: &MediaContent, options: &ProcessingOptions) -> Result<MediaContent> {
        ...
        #[cfg(feature = "image-support")]
        {
            ...
            if let Some(target_format) = &options.target_format {
                if target_format != &content.format {
                    ...
                    let converted_data = self.convert_image_format(&image, target_format.clone(), quality).await?;
            ...
            processed_content.features.extend(
                features.into_iter().map(|(k, v)| (k, v))
            );
        ...
        {
            Err(crate::error::MultimodalError::feature_not_enabled("image-support"))
        }
    }
...
mod tests {
    ...
    fn test_image_processor_creation() {
        ...
        assert_eq!(processor.supported_input_modalities(), vec![ModalityType::Image]);

...
impl AudioFeatureExtractor {
    ...
    pub async fn extract_features(&self, content: &MediaContent) -> Result<HashMap<String, serde_json::Value>> {
        ...
        if self.extract_spectral_features {
            self.extract_spectral_features_impl(content, &mut features).await?;
        }
        
        // Extract tempo features if enabled
        if self.extract_tempo_features {
            self.extract_tempo_features_impl(content, &mut features).await?;
        }
        
        // Extract harmonic features if enabled
        if self.extract_harmonic_features {
            self.extract_harmonic_features_impl(content, &mut features).await?;
        }
    ...
    pub async fn extract_basic_features(
        ...
        #[cfg(feature = "audio-support")]
        {
            ...
            match content.format {
                MediaFormat::Mp3 => {
                    ...
                    features.insert("duration".to_string(), self.estimate_duration(content).into());
                },
                MediaFormat::Wav => {
                    features.insert("sample_rate".to_string(), 44100.into());
                    features.insert("channels".to_string(), 2.into());
        ...
        #[cfg(not(feature = "audio-support"))]
        {
            ...
            features.insert("estimated_duration".to_string(), self.estimate_duration(content).into());
        }
...
mod tests {
    ...
    async fn test_extract_basic_features() {
        ...
        let mut features = HashMap::new();
        extractor.extract_basic_features(&content, &mut features).await.unwrap();

...
use crate::types::{MediaContent, MediaFormat, MediaData, MediaMetadata};
...
impl AudioLoader {
    ...
    pub async fn load_from_path<P: AsRef<Path>>(&self, path: P) -> Result<MediaContent> {
        ...
        let metadata = fs::metadata(path)
            .map_err(|e| MultimodalError::media_processing(format!("Failed to read file metadata: {}", e)))?;
        ...
        let data = fs::read(path)
            .map_err(|e| MultimodalError::media_processing(format!("Failed to read audio file: {}", e)))?;
    ...
    pub async fn load_from_bytes(&self, data: Vec<u8>, filename: Option<&str>) -> Result<MediaContent> {
        // Check file size
        if let Some(max_size) = self.max_file_size {
            if data.len() as u64 > max_size {
                return Err(MultimodalError::content_too_large(data.len() as u64, max_size));
            }
        }
    ...
    pub async fn load_from_url(&self, url: &str) -> Result<MediaContent> {
        ...
        let response = reqwest::get(url)
            .await
            .map_err(|e| MultimodalError::network(format!("Failed to fetch audio from URL: {}", e)))?;
        ...
        let filename = url.split('/').last().map(|s| s.split('?').next().unwrap_or(s));
...
mod tests {
    ...
    async fn test_load_from_bytes() {
        ...
        let content = loader.load_from_bytes(wav_data, Some("test.wav")).await.unwrap();

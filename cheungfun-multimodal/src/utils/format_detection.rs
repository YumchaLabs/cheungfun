    ...
    pub fn record_detection(&mut self, format: MediaFormat, confidence: f32) {
        ...
        *self.detections_by_modality.entry(modality).or_insert(0) += 1;
        ...
        let total_confidence =
            self.average_confidence * (self.total_detections - 1) as f32 + confidence;
...
mod tests {
    ...
    fn test_detect_format_from_path() {
        assert_eq!(
            detect_format_from_path("image.jpg"),
            Some(MediaFormat::Jpeg)
        );
        assert_eq!(
            detect_format_from_path("/path/to/audio.mp3"),
            Some(MediaFormat::Mp3)
        );
        assert_eq!(
            detect_format_from_path("document.pdf"),
            Some(MediaFormat::Pdf)
        );
    ...
    fn test_detect_modality_from_path() {
        assert_eq!(
            detect_modality_from_path("image.jpg"),
            Some(ModalityType::Image)
        );
        assert_eq!(
            detect_modality_from_path("audio.mp3"),
            Some(ModalityType::Audio)
        );
        assert_eq!(
            detect_modality_from_path("video.mp4"),
            Some(ModalityType::Video)
        );
        assert_eq!(
            detect_modality_from_path("document.pdf"),
    ...
    fn test_detect_text_format_from_content() {
        assert_eq!(
            detect_text_format_from_content(b"Hello, world!"),
            Some(MediaFormat::PlainText)
        );
        assert_eq!(
            detect_text_format_from_content(b"<!DOCTYPE html><html></html>"),
            Some(MediaFormat::Html)
        );
        assert_eq!(
            detect_text_format_from_content(b"<?xml version=\"1.0\"?>"),
            Some(MediaFormat::Xml)
        );
        assert_eq!(
            detect_text_format_from_content(b"{\"key\": \"value\"}"),
            Some(MediaFormat::Json)
        );
        assert_eq!(
            detect_text_format_from_content(b"# Markdown Header"),
            Some(MediaFormat::Markdown)
        );
        assert_eq!(
            detect_text_format_from_content(b"name,age,city\nJohn,30,NYC"),
            Some(MediaFormat::Csv)
        );
    }

    #[test]
    fn test_validate_format_modality() {
        assert!(validate_format_modality(
            MediaFormat::Jpeg,
            ModalityType::Image
        ));
        assert!(validate_format_modality(
    ...
    fn test_format_detection_confidence() {
        ...
        let unknown_confidence =
            format_detection_confidence(b"random", &MediaFormat::Unknown("test".to_string()));

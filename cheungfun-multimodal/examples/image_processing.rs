...
async fn main() -> Result<()> {
    ...
    let image_content = MediaContent::from_bytes(image_data, MediaFormat::Png).with_metadata(
        MediaMetadata::new()
    ...
    println!(
        "   - Size: {} bytes",
        image_content.estimated_size().unwrap_or(0)
    );
    println!(
        "   - Title: {}",
        image_content.metadata.title.as_deref().unwrap_or("N/A")
    );
    ...
    #[cfg(feature = "image-support")]
    {
        ...
        match processor.process(&image_content, &processing_options).await {
            Ok(processed_content) => {
                ...
                println!(
                    "   - New size: {} bytes",
                    processed_content.estimated_size().unwrap_or(0)
                );
            }
            Err(e) => {
    ...
    let multimodal_doc =
        MultimodalDocument::with_media(base_document, ModalityType::Image, image_content.clone())
    ...
    println!(
        "   - Has media: {}",
        multimodal_node.media_content.is_some()
    );
    println!(
        "   - Primary confidence: {:.2}",
        multimodal_node.get_primary_confidence().unwrap_or(0.0)
    );
    ...
    let detected_format =
        cheungfun_multimodal::utils::detect_format(&unknown_data, Some("test.png")).await;
    ...
    let requires_preprocessing =
        cheungfun_multimodal::utils::requires_preprocessing(&image_content);
...
mod tests {
    ...
    async fn test_multimodal_document_creation() {
        ...
        let multimodal_doc = MultimodalDocument::with_media(doc, ModalityType::Image, content);

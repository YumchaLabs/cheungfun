...
use cheungfun_agents::{
    agent::{
        ...
        multi_agent::{AgentRole, HandoffStrategy, MultiAgentOrchestratorBuilder},
...
async fn single_agent_calculator_example() -> Result<()> {
    ...
    let react_config = ReActConfig::new("Math Assistant").with_max_iterations(5);

    // Update the instructions to be math-specific
    let mut config = react_config;
    config.base_config.instructions = Some("You are a helpful math assistant. Use the calculator tool for any mathematical calculations.".to_string());
...
async fn research_assistant_example() -> Result<()> {
    ...
    let mut tool_registry = ToolRegistry::new();
    tool_registry.register(Arc::new(WikipediaTool::new()))?;
    tool_registry.register(Arc::new(CalculatorTool::new()))?;
    ...
    tool_registry.register(Arc::new(safe_shell))?;

    // Create research assistant agent
    let react_config = ReActConfig::new("Research Assistant").with_max_iterations(8);

    let mut config = react_config;
    config.base_config.description =
        Some("Expert research assistant with multiple tools".to_string());
...
async fn multi_agent_research_pipeline() -> Result<()> {
    ...
    let research_llm = SiumaiLlmClient::openai(
        std::env::var("OPENAI_API_KEY").expect("OPENAI_API_KEY must be set"),
        "gpt-4",
    )
    .await?;
    ...
    research_tools.register(Arc::new(CalculatorTool::new()))?;

    let mut research_config = ReActConfig::new("Research Agent").with_max_iterations(6);
    research_config.base_config.description = Some("Specialized research agent".to_string());
    research_config.base_config.instructions = Some(
        "You are a specialized research agent. Your role is to gather comprehensive, \
         ...
         When you have completed your research, indicate this by saying 'RESEARCH_COMPLETE:'"
            .to_string(),
    );

    let research_agent = Arc::new(ReActAgent::with_llm_client(
        research_config,
    ...
    let analysis_llm = SiumaiLlmClient::openai(
        std::env::var("OPENAI_API_KEY").expect("OPENAI_API_KEY must be set"),
        "gpt-4",
    )
    .await?;
    ...
    analysis_tools.register(Arc::new(CalculatorTool::new()))?;

    let mut analysis_config = ReActConfig::new("Analysis Agent").with_max_iterations(5);
    analysis_config.base_config.description = Some("Specialized analysis agent".to_string());
    analysis_config.base_config.instructions = Some(
    ...
    let analysis_agent = Arc::new(ReActAgent::with_llm_client(
        analysis_config,
        Arc::new(analysis_tools),
        analysis_llm,
    )) as Arc<dyn BaseAgent>;

    // 3. Writer Agent - Creates final summary
    let writer_llm = SiumaiLlmClient::openai(
        std::env::var("OPENAI_API_KEY").expect("OPENAI_API_KEY must be set"),
        "gpt-4",
    )
    .await?;
    ...
    let mut writer_config = ReActConfig::new("Writer Agent").with_max_iterations(3);
    writer_config.base_config.description = Some("Specialized writing agent".to_string());
    writer_config.base_config.instructions = Some(
        "You are a specialized writing agent. Your role is to take research data and analysis \
         ...
         and actionable insights. Create a final summary that would be useful for decision-making."
            .to_string(),
    );

    let writer_agent = Arc::new(ReActAgent::with_llm_client(
        writer_config,
        Arc::new(writer_tools),
        writer_llm,
    )) as Arc<dyn BaseAgent>;
